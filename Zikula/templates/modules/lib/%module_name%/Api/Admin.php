...
class %module_name%_Api_Admin extends Zikula_AbstractApi{
   ...
   */
	public function getlinks(){
        ...
        if (SecurityUtil::checkPermission('%module_name%::', '::', ACCESS_EDIT)) {
            $links[] = array(
                'url' => ModUtil::url('%module_name%', 'admin', 'main'),
                'text' => $this->__('Example Dropdown'),
                'class' => 'z-icon-es-view',
                'links' => array(
                    array('url'  => ModUtil::url('%module_name%', 'user', 'sitemap'),
                          'text' => $this->__('Sitemap')),
                    array('url'  => ModUtil::url('%module_name%', 'user', 'extlist'),
                          'text' => $this->__('Extended')),
                    array('url'  => ModUtil::url('%module_name%', 'user', 'pagelist'),
                          'text' => $this->__('Complete')),
                    array('url'  => ModUtil::url('%module_name%', 'user', 'categories'),
                          'text' => $this->__('Category list')),
                    array('url'  => ModUtil::url('%module_name%', 'admin', 'deletedpages'),
                          'text' => $this->__('Restore pages')),
                ));
        }
        if (SecurityUtil::checkPermission('%module_name%::', '::', ACCESS_ADD)) {
            $links[] = array(
                'url'   => ModUtil::url('%module_name%', 'admin', 'newPage'),
                'text'  => $this->__('Add new'),
                'class' => 'z-icon-es-new');
        }
        if (SecurityUtil::checkPermission('%module_name%::', '::', ACCESS_ADMIN)) {
            $links[] = array(
                'url'   => ModUtil::url('%module_name%', 'admin', 'settings'),
                'text'  => $this->__('Settings'),
                'class' => 'z-icon-es-config');
            $links[] = array(
                'url'   => ModUtil::url('%module_name%', 'admin', 'upgradecontenttypes'),
                'text'  => $this->__('Upgrade StoreTypes'),
                'class' => 'z-icon-es-gears');
            $links[] = array(
                'url'   => ModUtil::url('%module_name%', 'admin', 'migrate'),
