 ...
 *---------------------------------------------------------------------------*/
 ...
 http://url/app/%module_id%/view/type/grid
 ...
 */
class %module_name%_Controller_Grid extends Zikula_AbstractController {
  ...
  */
  ...
  // bypass illegal parameter to 'main'
    ...
    if ( !MyAksi_Util::isInternalCall( )) {
  	...
  	// error
    //echo %module_name%_Anggota_wow();
    //die;
  	$now = DateUtil::getDatetime();
  	$lsHTML = "<pre>;\n".print_r( $_REQUEST, true);
  	return $lsHTML;
	}
  //http://%module_name%/app/%module_id%/testobject/type/anggota
	public function testobject( $args){
  	$this->oDBConn = DBConnectionStack::init( "%module_id%");
  	$loAnggota = new %module_name%_DBObject_Anggota( null, "No_Anggota");
  ...
  public function view( $args){
    ...
    $laActionURL = array();
  	$laData = array (
    	"db.external" => true,
      "db.name" => %module_name%_Constant::DB_CONNECTION_NAME,
      "table.key" => $this->sPrimaryKey,
      "table.name" => $this->sTableName,
      "table.order" => "No_Anggota desc",
      "table.limit" => %module_name%_Constant::DATA_LIMIT,
